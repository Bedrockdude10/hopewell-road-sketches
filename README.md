...
| ebroad_princeton | `build_proposal_bike_lanes` | The same 5 ft section, asphalt painted green, both sides of both E Broad legs. **Protected on `e_broad_st_west` only**, and that asymmetry is the finding: a post needs the standard 2 ft buffer, and against the 18.82 ft a full section spends, west has 18.83 / 18.96 ft and east has 17.62 / 18.31 — short by 1.20 and 0.51. East's lane runs hard against the kerb with no hatched margin at all. Each unprotected kerb says so in a printed note. Princeton Ave gets no lane (4.1 ft spare, under AASHTO's 5 ft). No parking displaced — both E Broad legs are already `no_stopping`/`no_parking`. |
...
Two versions of this were wrong in ways worth keeping:
