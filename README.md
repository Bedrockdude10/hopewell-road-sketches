...
| broad_st_greenwood | `build_proposal_bike_lanes` | Buffered 6 ft lanes with a 3 ft buffer, flex posts down each buffer, both sides of both Broad legs. Asphalt painted green. **Not** parking-protected — see below. Greenwood Ave gets none (2.3 / 4.6 ft spare per side, under AASHTO's 5 ft). |
| ebroad_princeton | `build_proposal_bike_lanes` | 5 ft lanes, asphalt painted green, both sides of both E Broad legs. **Protected on `e_broad_st_west` only**, and that asymmetry is the finding: a post needs a buffer, a buffer needs 1.64 ft to hold its own two stripes, and against the 16.82 ft an 11 ft lane + a 5 ft lane + their inner stripes spend, west has 2.01 / 2.14 ft spare and east has 0.80 / 1.49. Each unprotected kerb says so in a printed note. Princeton Ave gets no lane at all (4.1 ft spare). No parking displaced — both E Broad legs are already `no_stopping`/`no_parking`. |
