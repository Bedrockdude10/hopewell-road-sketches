...
Test order is **shuffled every run** (pytest-randomly), which is how you find out whether the session-scoped `site_models` fixture is as read-only as it claims. A failure that only happens in one order is reproducible: the header prints `Using --randomly-seed=N`, and `./scripts/test.sh --randomly-seed=N` replays it. (`pytest.ini` sets `-q`, which hides that header — CI runs without it for exactly this reason.)

CI (`.github/workflows/tests.yml`) runs the lint, the import contracts and the suite on every push. It has no `data/`, so 236 of the tests skip there — every one that builds a real junction, **including the golden geometry comparison**. A green tick means the code is sound, not that the renders are. The goldens stay a local guard.
...
- **`tests/test_lint.py`** runs two static checkers. `ruff` (configured by `ruff.toml`) over every `.py` file, reporting undefined names separately from everything else because they are a guaranteed crash on whatever branch reaches them — the whole reason the file exists (an import collapse dropped `fetch_buildings`, referenced only under `--render-3d`, and it surfaced as a `NameError` inside a worker process). And `import-linter` (`.importlinter`) over the import *graph*, for the three rules no single file can show you: `scripts/blender/*` must not import this project or its venv (Blender runs them under its own interpreter, so a stray import fails minutes into a 3D render, inside a subprocess); reading a `config.yaml` must not drag in shapely and the geometry stack; and geometry must not import the output stages. Every `ignore` in `ruff.toml` and every contract in `.importlinter` carries the argument for it.
...
- **`tests/test_frame_properties.py`** is property-based (hypothesis) over the station/offset frame in `src/geometry/model/`. It asserts the transform's contracts on generated centerlines rather than on the four that exist, so a failure will name a shape nobody wrote down. It has already been useful for what it *disproved*: it established that a polyline kink makes a band of stations around the vertex genuinely unreachable at any offset, which is a fact about the geometry that the placement code works around rather than a defect.
...
2. Project it to feet **in `src/geometry/intersection/`**, store it as a frozen dataclass on the model, and give that dataclass the derived geometry every consumer wants (`Driveway.surface`, not `driveway_width_m` for each renderer to re-widen).
